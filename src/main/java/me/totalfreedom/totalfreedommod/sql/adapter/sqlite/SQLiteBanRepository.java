 ...
 * Uses SQLite-specific SQL syntax.
 */
public class SQLiteBanRepository implements BanRepository
{
    private final TotalFreedomMod plugin;
    private final StatementHandler statementHandler;

    public SQLiteBanRepository(TotalFreedomMod plugin, StatementHandler statementHandler)
    {
        this.plugin = plugin;
    ...
    public void insertIps(int banId, List<String> ips) throws SQLException
    {
        ...
        String sql = "INSERT OR IGNORE INTO ban_ips (ban_id, ip) VALUES (?, ?)";
        for (String ip : ips)
        {
    ...
    {
        // Active bans: expire_at is NULL (permanent) or expire_at > current time
        String sql = """
    ...
    public List<Ban> findExpiredBans() throws SQLException
    {
        String sql = """
            SELECT id, uuid, username, banned_by, banned_by_uuid, reason, expire_at
            FROM bans
            WHERE expire_at IS NOT NULL AND datetime(expire_at) <= datetime('now')
            """;
        List<Ban> bans = new ArrayList<>();
        try (ResultSet rs = statementHandler.executeQuery(sql))
        {
    ...
    public boolean isBanned(UUID uuid) throws SQLException
    {
        String sql = """
            ...
            WHERE uuid = ? AND (expire_at IS NULL OR datetime(expire_at) > datetime('now'))
            """;
        try (PreparedStatement stmt = statementHandler.prepareStatement(sql, uuid.toString());
             ResultSet rs = stmt.executeQuery())
        {
    ...
    public boolean isBannedByUsername(String username) throws SQLException
    {
        String sql = """
            SELECT COUNT(*) FROM bans 
            WHERE LOWER(username) = LOWER(?) AND (expire_at IS NULL OR datetime(expire_at) > datetime('now'))
            """;
        try (PreparedStatement stmt = statementHandler.prepareStatement(sql, username);
             ResultSet rs = stmt.executeQuery())
        {
    ...
    public boolean isBannedByIp(String ip) throws SQLException
    {
        String sql = """
            ...
            INNER JOIN ban_ips bi ON b.id = bi.ban_id
            WHERE bi.ip = ? AND (b.expire_at IS NULL OR datetime(b.expire_at) > datetime('now'))
            """;
        try (PreparedStatement stmt = statementHandler.prepareStatement(sql, ip);
             ResultSet rs = stmt.executeQuery())
        {
    ...
    {
        // First get the ban IDs for the given IP
    ...
    {
        String sql = "DELETE FROM bans WHERE expire_at IS NOT NULL AND datetime(expire_at) <= datetime('now')";
